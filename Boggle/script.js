	...
	for (let i=0; i<tiles.length; i++) {
		tiles[i].addEventListener("click", e => {
			...
			if (word.length == 0) {
				...
				searchable = [j-4,j-3,j+1,j+4,j+5]
					.filter(k => k > -1 && k < 16);
				...
				searchable = [j-5,j-4,j-1,j+3,j+4]
					.filter(k => k > -1 && k < 16);
				if (searchable.includes(i)) {
					word+=tile.textContent;
					searched.push(i);
				...
				searchable = [j-5,j-4,j-3,j-1,j+1,j+3,j+4,j+5]
						.filter(k => k > -1 && k < 16);
				if (searchable.includes(i)) {
					word+=tile.textContent;
					searched.push(i);
...
tiles.forEach(tile => { tile.children[0].style.visibility = "hidden"; });
...
function deadEnd(tile) {
	...
	if (tile.classList.contains("left-side")) {
		...
		for (let i=0; i<searchable.length; i++) 
			if (isPrefix(word+tiles[searchable[i]].textContent) 
				&& !searched.includes(searchable[i])) return false;
		...
		searchable = [index-5, index-4, index-3, index-1, index+1,
 ...
 */
function isPrefix(p) { return dict.isPrefix(p.toUpperCase()); }
...
function tallyScore() {
	// Not a word
	if (!isValidWord(word)) {
		searchedListHTML.insertAdjacentHTML("afterbegin",
...
function randomize() {
	...
	let possible = "aaafrsaaeeeeaafirsadennnaeeeemaeegmuaegmnna"+
...
function revealAll() {
	...
	for (let i=0; i<tiles.length; i++) revealAllHelper(i, {visited: []}, "");

	solutions.sort((a, b) => { return b.length - a.length; });
...
function revealAllHelper(index, searched, prefix) {
	...
	if (dict.isPrefix(prefix.toUpperCase())) {
		...
		if (tile.classList.contains("left-side")) {
			searchable = [index-4, index-3, index+1,index+4, index+5]
				.filter(i => i > -1 && i < 16)
				.forEach(i => {revealAllHelper(i, searched, prefix);});
		} else if (tile.classList.contains("right-side")) {
